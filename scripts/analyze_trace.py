...
def parse_trace_file(filepath: str) -> Dict[Tuple[str, str], MethodStats]:
    """Parse trace file and collect method statistics with self-time calculation."""
    ...
    with open(filepath, 'r') as f:
        for line in f:
            ...
            if exit_match:
                ...
                if key not in stats:
                    stats[key] = MethodStats(class_name, method_name)
                stats[key].record(duration_us, self_time_us)
...
def print_summary(stats: Dict[Tuple[str, str], MethodStats], sort_by: str = 'self'):
    """Print summary table similar to Rust tracer output."""
    ...
    print()
    print("=" * 120)
    print(" Method Timing Summary ")
    print("=" * 120)
    print()
    print(f"Total method exits processed: {total_events}")
    print(f"Unique methods traced: {len(stats_list)}")
    print(f"Total self time: {self_total/1000:.2f}ms")
    print()
    ...
    # if len(stats_list) > top_n:
    #     print(f"\n... and {len(stats_list) - top_n} more methods")
...
def main():
    ...
    stats = parse_trace_file(args.trace_file)
    
    if args.filter:
        pattern = re.compile(args.filter)
        stats = {k: v for k, v in stats.items() if pattern.search(k[0]) or pattern.search(k[1])}
    ...
    if ignore_patterns:
        ...
        print(f"Filtered out {before - len(stats)} methods")
    
    print_summary(stats, sort_by=args.sort_by)
